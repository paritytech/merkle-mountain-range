...
)
{
    ...
    let proof = if let Some(proof_positions) = handrolled_proof_positions {
        MerkleProof::new(11, proof_positions.iter().map(|pos| mmr.store().get_elem(*pos).unwrap().unwrap()).collect())
    }
    else {mmr.gen_proof(if positions_only_peaks {positions_to_verify}
