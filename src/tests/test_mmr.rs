...
fn test_invalid_proof_verification(
    ...
    let tampered_proof: Option<MerkleProof<MyItem, MyMerge>> = if let Some(tampered_proof_positions) = handrolled_tampered_proof_positions {
        Some(
            MerkleProof::new(mmr.mmr_size(),
    ...
    let positions_only_peaks = positions_to_verify
        .iter()
        .all(|pos| get_peaks(mmr.mmr_size()).contains(pos));
    let proof = if let Some(proof_positions) = handrolled_proof_positions {
        ...
        mmr.gen_proof(if positions_only_peaks {
            positions_to_verify
        } else {
            positions_to_verify
                .iter()
                .map(|pos| *pos)
                .filter(|pos| {
                    let peaks = get_peaks(mmr.mmr_size());
                    if let Ok(peak_index) = peaks.binary_search(pos) {
                        !positions_to_verify.iter().any(|pos| {
                            if peak_index == 0 {
                                pos < &peaks[peak_index]
                            } else {
