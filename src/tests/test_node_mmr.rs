...
fn test_invalid_proof_verification(
    ...
    let tampered_proof: Option<NodeMerkleProof<MyItem, MyMerge>> =
        handrolled_tampered_proof_positions.map(|tampered_proof_positions| {
            NodeMerkleProof::new(
                mmr.mmr_size(),
    ...
    assert_eq!(proof.verify(root.clone(), entries_to_verify), Ok(true));
    // if any nodes to be verified aren't members of the mmr, the proof verification fails
    let tampered_entries_result = proof.verify(root, tampered_entries_to_verify.clone());
